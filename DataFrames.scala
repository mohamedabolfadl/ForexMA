...
object DataFrames {
  ...
  def mapper(line:String): Price = {
   ...
   def getMATF_T(sp: SparkSession, cross: String, MAv: Int, pipVal:Double, sgnCross:Double, cr:String, timeFrame:Int, crossName:String): DataFrame = {
    ...
    val quotesRaw = lines.map(mapperTup).toDF(strName,"date","close").persist(MEMORY_ONLY_SER)
    //val quotes = quotesRaw.filter($"minu"%timeFrame<1).drop(quotesRaw.col("minu")).persist(MEMORY_ONLY_SER)
    val quotes = quotesRaw.filter(quotesRaw(strName)%timeFrame<1).persist(MEMORY_ONLY_SER)
    
    val wSpec1 = Window.rowsBetween(-(MAv-1), 0)
    val quotesWithSMA = quotes.withColumn("SMA",avg(quotes("close")).over(wSpec1)  ).persist(MEMORY_ONLY_SER)
   if (sgnCross<0)
   {
  ...
  def main(args: Array[String]) {
    ...
    val cadStrength = getMATF(spark, "../USDCADs.csv", MAv, 0.0001, 1,"CAD",1,"USDCAD").persist(MEMORY_ONLY_SER)
    val gbpStrength = getMATF(spark, "../GBPUSDs.csv", MAv, 0.0001, -1,"GBP",1,"GBPUSD").persist(MEMORY_ONLY_SER)
    val jpyStrength = getMATF(spark, "../USDJPYs.csv", MAv, 0.01, 1,"JPY",1,"USDJPY").persist(MEMORY_ONLY_SER)
    val nzdStrength = getMATF(spark, "../NZDUSDs.csv", MAv, 0.0001, -1,"NZD",1,"NZDUSD").persist(MEMORY_ONLY_SER)
    val chfStrength = getMATF(spark, "../USDCHFs.csv", MAv, 0.0001, 1,"CHF",1,"USDCHF").persist(MEMORY_ONLY_SER)
    val audStrength = getMATF(spark, "../AUDUSDs.csv", MAv, 0.0001, -1,"AUD",1,"AUDUSD").persist(MEMORY_ONLY_SER)
  ...
  val averageTotalWithPreviousMinute = averageTotal.withColumn("PrevTime",lag(averageTotal("USDCADminu"),1).over(wSpec1)).withColumn("PrevDate",lag(averageTotal("date"),1).over(wSpec1)).persist(MEMORY_ONLY_SER)
...
dfRenamed.unpersist()
 ...
 // Removing the temporary column of the minute field
  //val averageTotal_filt = averageTotal.drop("USDCADminu").persist(MEMORY_ONLY_SER)
  ...
  val extracted_TF = completeData.filter($"USDCADminu"%timeFrame===0).persist(MEMORY_ONLY_SER)
completeData.unpersist()
   println("filtered")
